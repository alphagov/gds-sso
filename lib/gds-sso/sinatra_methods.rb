  ...
  module SSO
    module SinatraMethods
      ...
      def self.included(base)
        base.error PermissionDeniedException do |e|
          # For Karl - see this is an example of where Rails would render here but the Sinatra equivalent is halt(status, message)
          halt(401, "Actions to write to rummager requires edit publications authentication")
          #   KB - It looks like this in controller methods
          #       Dunno if we want to support the GDS::SSO::Config.api_only? - I _think_ rummmager is the only sinatra app so maybe it's not necessary?
          #
