...
describe GDS::SSO::MockBearerToken do
  describe ".locate" do
    ...
    it "doesn't modify the permissions of GDS::SSO.test_user" do
      ...
      allow(GDS::SSO).to receive(:test_user).and_return(test_user)
      allow(GDS::SSO::Config).to receive(:additional_mock_permissions_required)
        .and_return("extra_permission")
    ...
    it "uses the addition_mock_permissions_required option to add extra permissions" do
      test_user = TestUser.new
      allow(GDS::SSO::Config).to receive(:user_klass).and_return(TestUser)
      allow(TestUser).to receive(:where).and_return([test_user])
      allow(GDS::SSO::Config).to receive(:additional_mock_permissions_required)
        .and_return(%w[array of permissions])

      test_user = described_class.locate("anything")
      expect(test_user.permissions).to match_array(%w[signin array of permissions])
    end

    it "uses the addition_mock_permissions_required option to overwrite any existing permissions" do
